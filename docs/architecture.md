...
**Ordem de execução acordada com o usuário:** banco → LLM → autenticação → voz, migrando e testando cada peça isoladamente antes de seguir para a próxima (mesmo ritmo usado no resto do projeto). ✅ Banco (2026-08-12) e LLM (2026-08-12) concluídos. Próximo: autenticação (Clerk → `fastapi-users`).
